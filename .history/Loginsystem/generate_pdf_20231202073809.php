    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            if ($driverInfoResult->num_rows > 0) {
                ...
                $pdf->Cell(90, 10, "Complainee: $complaineeName", 1, 0, 'C'); // First cell in the first row

                // Add another cell to the right (adjust the x-coordinate)
                $pdf->Cell(90, 10, 'Additional Information', 1, 0, 'C'); // Second cell in the first row
                
                // Move to the next line
                $pdf->Ln();
                
                // Continue with the rest of the cells as before
                $pdf->Cell(90, 10, "Vehicle Number: " . $driverInfo['Age'], 1, 1, 'C'); // First cell in the second row
                $pdf->Cell(90, 10, "Plate Number: " . $driverInfo['Password'], 1, 0, 'C'); // Second cell in the second row
                $pdf->Cell(90, 10, "PhoneNumber: " . $driverInfo['PhoneNumber'], 1, 1, 'C'); // First cell in the third row
                $pdf->Cell(90, 10, "Address: " . $driverInfo['HomeAddress'], 1, 0, 'C'); // Second cell in the third row
