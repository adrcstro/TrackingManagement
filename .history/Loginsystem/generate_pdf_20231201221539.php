...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            $pdf->Cell(50, 10, 'Complainee Details', 1, 0, 'C', 1);
            // Second column with color (width: 70)
            $pdf->Cell(140, 10, 'Proof of Identity', 1, 1, 'C', 1);
            ...
            $driverInfoQuery = "SELECT Age, Password, PhoneNumber, HomeAddress FROM driverstbl WHERE DriverName = '$complaineeName'";
            ...
            if ($driverInfoResult->num_rows > 0) {
                ...
                $pdf->Cell(50, 10, "Complainee: $complaineeName", 1, 1, 'C'); // Move to the next line
                $pdf->Cell(50, 10, "Age: " . $driverInfo['Age'], 1, 0, 'C');
                $pdf->Cell(50, 10, "Password: " . $driverInfo['Password'], 1, 0, 'C');
                $pdf->Cell(50, 10, "PhoneNumber: " . $driverInfo['PhoneNumber'], 1, 0, 'C');
                $pdf->Cell(50, 10, "HomeAddress: " . $driverInfo['HomeAddress'], 1, 1, 'C'); // Move to the next line
