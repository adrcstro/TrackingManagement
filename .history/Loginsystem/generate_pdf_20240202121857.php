...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            // Assuming you have already instantiated a TCPDF object, e.g., $pdf
...
{
    global $pdf;
    $pdf->setTextShadow(2, 2, 4, array(0, 0, 0)); // Adjust parameters as needed
    $pdf->Cell(0, 0, $text, 0, 1, 'L');
    $pdf->setTextShadow(); // Reset text shadow
}

// Apply text shadow to the content
applyTextShadow("I am writing to formally bring to your attention a matter that requires your immediate Action. Complaint ID: " . $row['ComplaintID'] . ". This report pertains to a " . $row['TypeofComplaint'] . " that transpired on " . $row['DateofReport'] . ". The incident involves ");
applyTextShadow("a complainant identified as " . $row['ComplainantName'] . ". The complainant, residing at " . $row['Address'] . " and reachable at " . $row['ContactNumber'] . ", reported an incident involving the complainee, " . $row['NameofComplainee'] . ".\n\n");

applyTextShadow("In providing a comprehensive account of the incident, the complainant details are as follows:\n");
applyTextShadow("Name: " . $row['ComplainantName'] . ". ");
applyTextShadow("Contact Number: " . $row['ContactNumber'] . ". ");
applyTextShadow("Address: " . $row['Address'] . ".\n\n");

applyTextShadow("I would like to furnish you with the following details: ");
applyTextShadow($row['IncidentDescription'] . ". Subsequent to this, a diligent investigation has been conducted. Actions taken during the investigation include ");
applyTextShadow("documenting statements, gathering evidence, and interviewing relevant parties. This report encapsulates the details of the complaint, our investigative efforts, and the actions taken thus far.\n\n");

applyTextShadow("We appreciate your prompt attention to this matter and anticipate your cooperation in resolving the issues presented herein. Thank you for your commitment to maintaining the integrity and safety within our community.\n\n");
