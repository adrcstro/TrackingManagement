...
if (isset($_GET['search'])) {
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            echo '<a href="' . $row["DriversLicense"] . '" target="_blank"><button>View Drivers License</button></a>';
            ...
            echo '<a href="' . $row["VehicleRegistration"] . '" target="_blank"><button>View Vehicle Registration</button></a>';
            echo '<p><input type="text" value="Permit to Operate: ' . $row["PermittoOperate"] . '" style="border: 1px solid #ccc; padding: 5px; margin-bottom: 10px; border-radius: 5px; width: 300px;" readonly></p>';
            echo '<a href="' . $row["PermittoOperate"] . '" target="_blank"><button>View Permit to Operate</button></a>';
        }
    } else {
