...
<script>
    // Assuming you have the requestedUsername available in JavaScript
    // For example, you could get it from a cookie, local storage, or another source.
    var requestedUsername = "your_requested_username"; // Replace with the actual username

    // Use AJAX to fetch the data from the server
    var xhr = new XMLHttpRequest();
    xhr.open('GET', 'fetch_passengers.php?Username=' + requestedUsername, true);
    xhr.onreadystatechange = function () {
        if (xhr.readyState == 4 && xhr.status == 200) {
            var passengers = JSON.parse(xhr.responseText);
