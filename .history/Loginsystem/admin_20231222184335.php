    ...
    <?php
        ...
        if ($result) {
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                echo '<div class="col-md-4 mb-3">'; // Adjusted margin-bottom to mb-3 for a smaller margin
                echo '<div class="card">';
                echo '<img src="uploads/' . $image . '" class="card-img-top" alt="Card Image">';
                echo '<div class="card-body">';
                echo '<h5 class="card-title">' . $header . '</h5>';
                echo '<p class="card-text mt-1 mb-1"><small class="text-muted">' . $time . ' | ' . $date . '</small></p>'; // Adjusted to mt-1 and mb-1 for smaller margins
                $trimmed_body = strlen($body) > 90 ? substr($body, 0, 90) . '...' : $body;
                echo '<p class="card-text mt-1">' . $trimmed_body . ' <a href="#" class="read-more" data-toggle="modal" data-target="#readMoreModal' . $row['NewsID'] . '">Read More</a></p>'; // Adjusted to mt-1 for a smaller top margin
                echo '</div>';
                echo '</div>';
                echo '</div>';
