...
<button id="Delete"  type="button" class="btn btn-danger btn-sm m-1">
...
// Check if the delete button is clicked and if the Admin ID is provided
if (isset($_GET['Delete'])) {
    $delete_id = $_GET['delete_id'];
    // SQL to delete a record
    $sql = "DELETE FROM admintbl WHERE Admin = '$delete_id'";
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        echo '<tr>
                ...
                <td>' . $row["DateCreated"] . '</td>
                <td>
                    <button type="button" class="btn btn-warning btn-sm m-1" onclick="deleteRow(' . $row["Admin"] . ')">
                        <i class="bi bi-pencil"></i> Update
                    </button>
                </td>
            </tr>';
...
<script>
    function deleteRow(id) {
        if (confirm('Are you sure you want to delete this record?')) {
            window.location.href = 'your_php_file.php?delete_id=' + id;
        }
    }
</script>
