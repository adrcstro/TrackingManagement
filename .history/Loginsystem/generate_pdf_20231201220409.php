...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            $driverInfoQuery = "SELECT Age, Password, PhoneNumber, HomeAddress FROM driverstbl WHERE NameofComplainee = '$complaineeName'";
            $driverInfoResult = $conn->query($driverInfoQuery);
        
            if ($driverInfoResult->num_rows > 0) {
                $driverInfo = $driverInfoResult->fetch_assoc();
        
                // Update the table cell with the fetched data
                $pdf->Cell(50, 60, "Age: " . $driverInfo['Age'] . "\nPassword: " . $driverInfo['Password'] . "\nPhoneNumber: " . $driverInfo['PhoneNumber'] . "\nHomeAddress: " . $driverInfo['HomeAddress'], 1, 0, 'C');
            } else {
                $pdf->Cell(50, 60, "No details found for $complaineeName", 1, 0, 'C');
            }
            // First row, second column without color (width: 70)
            $pdf->Cell(140, 60, 'Table Data 2', 1, 1, 'C');
