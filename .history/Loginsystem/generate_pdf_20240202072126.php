...
$addressLength = strlen($driverInfo['HomeAddress']);

// Define the maximum length before reducing font size
$maxAddressLength = 20;

// Check if the address length exceeds the maximum
if ($addressLength > $maxAddressLength) {
    // Calculate the reduction factor based on the excess length
    $reductionFactor = $addressLength / $maxAddressLength;
...
// Output the cell with adjusted font size
$pdf->Cell(90, 10, "Address: " . $driverInfo['HomeAddress'], 1, 1, 'C');

// Reset the font size to its original value if needed
