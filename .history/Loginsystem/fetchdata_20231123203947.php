...
// Fetch ratings from the Rate column
$sql = "SELECT Rate, COUNT(*) as count FROM rating GROUP BY Rate";
$result = $conn->query($sql);

// Initialize an array to store the ratings
$ratings = array(
    'Unsatisfactory/Very Poor' => 0,
    'Poor' => 0,
    'Satisfactory/Fair' => 0,
    'Good' => 0,
    'Excellent' => 0,
...
// Process the results and update the ratings array
while ($row = $result->fetch_assoc()) {
    $ratings['Unsatisfactory/Very Poor'] += ($row['Rate'] == 1) ? $row['count'] : 0;
    $ratings['Poor'] += ($row['Rate'] == 2) ? $row['count'] : 0;
    $ratings['Satisfactory/Fair'] += ($row['Rate'] == 3) ? $row['count'] : 0;
    $ratings['Good'] += ($row['Rate'] == 4) ? $row['count'] : 0;
    $ratings['Excellent'] += ($row['Rate'] == 5) ? $row['count'] : 0;
}
...
$percentages = array(
    'Unsatisfactory/Very Poor' => ($ratings['Unsatisfactory/Very Poor'] / $total) * 100,
    'Poor' => ($ratings['Poor'] / $total) * 100,
    'Satisfactory/Fair' => ($ratings['Satisfactory/Fair'] / $total) * 100,
    'Good' => ($ratings['Good'] / $total) * 100,
    'Excellent' => ($ratings['Excellent'] / $total) * 100,
);

// Generate the JavaScript code with the calculated percentages
$jsCode = '<div style="padding: 50px;">
    <div data-stepped-bar=\'{"title": "Service Rating", "catagories": [';

foreach ($percentages as $category => $percentage) {
    // Assume getCategoryColor is a function that returns the color based on the category
    $jsCode .= '{ "name": "' . $category . '", "value": ' . round($percentage) . ', "color": "' . $color . '" }, ';
}

// Remove the trailing comma and complete the JavaScript code
$jsCode = rtrim($jsCode, ', ') . ']}\'></div></div>';
