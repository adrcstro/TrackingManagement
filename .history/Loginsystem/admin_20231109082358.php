...
<div class="modal" id="Drivertrack">
    <div class="modal-dialog modal-lg"> <!-- Add the 'modal-lg' class to make the modal wider -->
        ...
        <input type="text" id="age" name="age" value="<?php echo $row['Age']; ?>" readonly><br><br>

        <label for="password">Password</label>
        <input type="text" id="password" name="password" value="<?php echo $row['Password']; ?>" readonly><br><br>

        <label for="driver-license">Driver's License</label>
        <input type="text" id="driver-license" name="driver-license" value="<?php echo $row['DriversLicense']; ?>" readonly><br><br>

        <label for="vehicle-registration">Vehicle Registration</label>
        <input type="text" id="vehicle-registration" name="vehicle-registration" value="<?php echo $row['VehicleRegistration']; ?>" readonly><br><br>

        <label for="permit-to-operate">Permit to Operate</label>
        <input type="text" id="permit-to-operate" name="permit-to-operate" value="<?php echo $row['PermittoOperate']; ?>" readonly><br><br>

        <label for="phone-number">Phone Number</label>
        <input type="text" id="phone-number" name="phone-number" value="<?php echo $row['PhoneNumber']; ?>" readonly><br><br>

        <label for="home-address">Home Address</label>
        <input type="text" id="home-address" name="home-address" value="<?php echo $row['HomeAddress']; ?>" readonly><br><br>
    </div>
            </div>
            <div class="modal-footer">
...
if (isset($_POST['SelectDriver'])) {
    ...
    // Fetch data based on the selected driver
    $sql = "SELECT Age, Password, DriversLicense, VehicleRegistration, PermittoOperate, PhoneNumber, HomeAddress FROM driverstbl WHERE Username = '$selectedDriver'";
    $result = $conn->query($sql);

    if ($result->num_rows > 0) {
        // Output data of each row
        while($row = $result->fetch_assoc()) {
            echo "Age: " . $row["Age"]. " - Password: " . $row["Password"]. " - DriversLicense: " . $row["DriversLicense"]. " - VehicleRegistration: " . $row["VehicleRegistration"]. " - PermittoOperate: " . $row["PermittoOperate"]. " - PhoneNumber: " . $row["PhoneNumber"]. " - HomeAddress: " . $row["HomeAddress"]. "<br>";
        }
    } else {
        echo "0 results";
    }
...
<script>
    $(document).ready(function(){
        $("#trackButton").click(function(){
            var selectedDriver = $("#SelectDriver").val();
            $.ajax({
                type: "POST",
                url: "driverupdate.php",
                data: { SelectDriver: selectedDriver },
                success: function(response){
                    // Assuming that the response from driverupdate.php is the HTML with fetched data
