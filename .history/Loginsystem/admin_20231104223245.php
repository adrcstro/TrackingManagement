...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Make sure to replace 'your_db_hostname', 'your_db_username', 'your_db_password', and 'your_db_name' with your actual database credentials.
    $conn = new mysqli('your_db_hostname', 'your_db_username', 'your_db_password', 'your_db_name');

    if ($conn->connect_error) {
        die("Connection failed: " . $conn->connect_error);
    }

    $brgyAdmin = $_POST['input1'];
    $username = $_POST['input2'];
    $password = $_POST['input3'];
     $date = $_POST['datePicker'];
    $sql = "INSERT INTO admintbl (BRGY-Admin, Username, Password, DateCreated) VALUES ('$brgyAdmin', '$username', '$date', '$password')";

    if ($conn->query($sql) === TRUE) {
        // Add the SweetAlert message here
