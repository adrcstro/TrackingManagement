...
if (isset($_GET['Username'])) {
    ...
    if ($resultDriverName->num_rows > 0) {
        ...
        $sqlRating = "SELECT DriverName, Rate, Comments FROM rating WHERE DriverName = ?";
        ...
        if ($resultRating->num_rows > 0) {
            ...
            while ($row = $resultRating->fetch_assoc()) {
                ...
                $numericRatings[] = [$customer, $numericRating, $comments];
            }
        } else {
...
// Extract labels, data, and comments for Chart.js
...
var comments = ratingsData.map(function(item) {
    return item[2];
});
...
var barChart = new Chart(ctx, {
    ...
    data: {
        ...
        datasets: [{
            ...
            data: data,
            comments: comments,  // Add comments to the dataset
            backgroundColor: '#E48F45', // Set the bar color
            borderColor: '#E48F45',
            borderWidth: 1,
        }]
    },
        ...
        tooltips: {
            callbacks: {
                label: function(context) {
                    ...
                    // Add comments to the tooltip
                    var comments = context.dataset.comments[context.dataIndex];
                    if (comments) {
                        label += ' (' + comments + ')';
                    }
                    return label;
