...
<div class="modal" id="DriversRatings">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Drivers Personal Rating Scale</h4>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (localStorage.getItem("driversRatingsModalOpenedManually")) {
        openModal();
    }

    // Check if the modal should remain open after a refresh
    if (localStorage.getItem("driversRatingsModalRemainOpen")) {
        $('#DriversRatings').modal('show');
    }
});

function openModal() {
    // Close the modal if it's open
    $('#DriversRatings').modal('hide');

    // Set a flag in localStorage to indicate that the modal has been opened manually
    localStorage.setItem("driversRatingsModalOpenedManually", "true");

    // Open the modal after a short delay (you can adjust the delay if needed)
    setTimeout(function() {
        $('#DriversRatings').modal('show');
    }, 500);
}

// Hook into the modal's hidden.bs.modal event to store the state when the modal is closed
$('#DriversRatings').on('hidden.bs.modal', function () {
    // Set a flag in localStorage to indicate that the modal should remain closed
    localStorage.setItem("driversRatingsModalRemainOpen", "false");
});

// Hook into the modal's show.bs.modal event to store the state when the modal is opened
$('#DriversRatings').on('show.bs.modal', function () {
    // Set a flag in localStorage to indicate that the modal should remain open
    localStorage.setItem("driversRatingsModalRemainOpen", "true");
});
