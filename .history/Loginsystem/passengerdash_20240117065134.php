...
if (isset($_GET['username'])) {
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            // Display passenger information as before
        }
    } else {
        echo "0 results";
    }
...
// Additional code to fetch information based on selected passenger from the dropdown
if (isset($_POST['SelectPassenger'])) {
    ...
    $sql = "SELECT Name, Age, Gender, Phone, HomeAddress, Username FROM passengertbl WHERE Name = ?";
    $stmt = $conn->prepare($sql);
    ...
    $stmt->bind_param("s", $selectedPassenger);
    $stmt->execute();
    $result = $stmt->get_result();

    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            // Display information for the selected passenger
            // You can use similar HTML output as before to display the information
        }
    } else {
        echo "0 results for the selected passenger";
    }
