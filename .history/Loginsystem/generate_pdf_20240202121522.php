...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            // Function to add text shadows to each letter
            function addTextShadow($text)
            {
                $shadowedText = "";
                $letterContent = str_split($text);
                foreach ($letterContent as $letterContent) {
                    $shadowedText .= '<span style="text-shadow: 1px 1px 2px #000;">' . $letterContent . '</span>';
                }
                return $shadowedText;
            }
            
            $letterContent .= addTextShadow("I am writing to formally bring to your attention a matter that requires your immediate Action. Complaint ID: " . $row['ComplaintID'] . ". This report pertains to a " . $row['TypeofComplaint'] . " that transpired on " . $row['DateofReport'] . ". The incident involves ");
            $letterContent .= addTextShadow("a complainant identified as " . $row['ComplainantName'] . ". The complainant, residing at " . $row['Address'] . " and reachable at " . $row['ContactNumber'] . ", reported an incident involving the complainee, " . $row['NameofComplainee'] . ".\n\n");
            
            $letterContent .= addTextShadow("In providing a comprehensive account of the incident, the complainant details are as follows:\n");
            $letterContent .= addTextShadow("Name: " . $row['ComplainantName'] . ". ");
            $letterContent .= addTextShadow("Contact Number: " . $row['ContactNumber'] . ". ");
            $letterContent .= addTextShadow("Address: " . $row['Address'] . ".\n\n");
            
            $letterContent .= addTextShadow("I would like to furnish you with the following details: ");
            $letterContent .= addTextShadow($row['IncidentDescription'] . ". Subsequent to this, a diligent investigation has been conducted. Actions taken during the investigation include ");
            $letterContent .= addTextShadow("documenting statements, gathering evidence, and interviewing relevant parties. This report encapsulates the details of the complaint, our investigative efforts, and the actions taken thus far.\n\n");
            
            $letterContent .= addTextShadow("We appreciate your prompt attention to this matter and anticipate your cooperation in resolving the issues presented herein. Thank you for your commitment to maintaining the integrity and safety within our community.\n\n");
