...
// Your provided div values
$divValues = '{"title": "Service Rating", "categories": [ { "name": "Unsatisfactory/Very Poor", "value": 10, "color": "#ff6384" }, { "name": "Poor", "value": 50, "color": "#ff9f40" },{ "name": "Satisfactory/Fair", "value": 10, "color": "#ffcd56" }, { "name": "Good", "value": 10, "color": "#4bc0c0" }, { "name": "Excellent", "value": 5, "color": "#36a2eb" } ]}';

// Decode JSON string
$divValuesArray = json_decode($divValues, true);

// Assuming you have a column named 'Rate' in your 'rating' table
$data = array();

foreach ($divValuesArray['categories'] as $category) {
    $name = $category['name'];

    // Fetch the records from the 'rating' table
    $sql = "SELECT Rate FROM rating WHERE Name = '$name'";
    ...
    if ($result->num_rows > 0) {
        ...
        $data[] = null; // or any default value if the record is not found
    }
}
