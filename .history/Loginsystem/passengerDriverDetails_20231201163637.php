...
<head>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            var selectButtons = document.querySelectorAll('.btn-outline-secondary');

            // Attach a click event listener to each button
            selectButtons.forEach(function (button) {
                button.addEventListener('click', function () {
                    // Find the associated input field
                    var inputField = button.parentElement.querySelector('.form-control');

                    // Create a ClipboardItem with the input field's value
                    var clipboardItem = new ClipboardItem({ "text/plain": new Blob([inputField.value], { type: "text/plain" }) });

                    // Use the Clipboard API to copy the item to the clipboard
                    navigator.clipboard.write([clipboardItem]);

                    // Notify the user (you can customize this part)
                    alert('Data copied: ' + inputField.value);
                });
            });
        });
    </script>
