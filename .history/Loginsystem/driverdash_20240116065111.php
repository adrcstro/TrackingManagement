...
<script>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <label for="NewsID">Confirm your Driver ID</label>
...
if (isset($_GET['username'])) {
    ...
    // Fetching the ID for the logged-in user
    $sql = "SELECT ID FROM driverstbl WHERE Username = ?";
    $stmt = $conn->prepare($sql);
    ...
    if ($result->num_rows > 0) {
        ...
        $stmt->close();
        $conn->close();
        exit; // Exit if no results for the logged-in user
    }

    $stmt->close();

    // Fetching all IDs for display in the dropdown
    $sqlAllIds = "SELECT ID FROM driverstbl";
    $resultAllIds = $conn->query($sqlAllIds);

    if ($resultAllIds->num_rows > 0) {
        echo '<select name="userIds">';
        while ($rowAllIds = $resultAllIds->fetch_assoc()) {
            $selected = ($rowAllIds["ID"] == $loggedInUserId) ? 'selected' : '';
            echo '<option value="' . $rowAllIds["ID"] . '" ' . $selected . '>' . $rowAllIds["ID"] . '</option>';
        }
        echo '</select>';
    } else {
        echo "0 results for all users";
    }
