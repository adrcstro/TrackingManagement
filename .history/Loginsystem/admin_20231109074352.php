...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Query the database based on the selected driver
    $sql = "SELECT * FROM driverstbl WHERE Username='$selectedDriver'";
    $result = $conn->query($sql);
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo "Driver ID: " . $row["id"] . "<br>";
            echo "Driver Name: " . $row["Username"] . "<br>";
...
<script>
    document.getElementById('trackButton').addEventListener('click', function () {
        ...
        xhr.open('POST', 'driverupdate.php', true);
        xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');

        // Send the request over the network
        xhr.send('SelectDriver=' + selectedDriver);
        ...
        xhr.onload = function () {
            if (xhr.status != 200) { // analyze HTTP response status
                ...
                alert(`Driver information: ${xhr.responseText}`);
            }
        };
