    ...
    <?php
        ...
        if ($result) {
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                echo '</div>';
                echo '<h5 class="card-title mb-1">' . $header . '</h5>'; // Adjust margin-bottom for the card title
                $trimmed_body = strlen($body) > 90 ? substr($body, 0, 90) . '...' : $body;
                echo '<p class="card-text">' . $trimmed_body . ' <a href="#" class="read-more" data-toggle="modal" data-target="#readMoreModal' . $row['NewsID'] . '">Read More</a></p>'; // Reduce margin-top for the card text
                echo '</div>';
                echo '</div>';
                echo '</div>';
