...
// Fetching Driver IDs
$sql = "SELECT ID FROM driverstbl";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    echo '<label for="driverSelect">Select Driver ID:</label>';
    echo '<select id="driverSelect" name="driverId">';
    while($row = $result->fetch_assoc()) {
        echo '<option value="'.$row["ID"].'">'.$row["ID"].'</option>';
    }
    echo '</select>';
    echo '<input type="submit" value="Fetch Details" name="fetchDetails">';
} else {
    echo "0 results";
}
