...
document.getElementById('button-addon2').addEventListener('click', function() {
    ...
    var driverName = "John Doe"; // Replace with your method to get the driver name dynamically

    // Send an AJAX request to the server to update the search bar input
    var xhr = new XMLHttpRequest();
    xhr.onreadystatechange = function() {
        if (xhr.readyState == 4 && xhr.status == 200) {
            // Update the search bar input
            document.getElementById('PassSearchDriver').value = "Driver Name: " + driverName;
        }
    };
    xhr.open('GET', 'updateSearchBar.php?driverName=' + encodeURIComponent(driverName), true);
    xhr.send();
});
</script>
