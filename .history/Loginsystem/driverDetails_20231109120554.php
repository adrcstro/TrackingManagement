...
if (isset($_GET['search'])) {
    ...
    $sql = "SELECT Username, Age, Password, PhoneNumber, HomeAddress,DriversLicense,VehicleRegistraton,PermittoOperate FROM driverstbl WHERE Username LIKE '%$searchText%' OR Age LIKE '%$searchText%' OR Password LIKE '%$searchText%'";
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            echo '<p><input type="text" value="Vehicle Registration: ' . $row["VehicleRegistraton"] . '" style="border: 1px solid #ccc; padding: 5px; margin-bottom: 10px; border-radius: 5px; width: 300px;" readonly></p>';
            echo '<p><input type="text" value="Permi to Operate: ' . $row["PermittoOperate"] . '" style="border: 1px solid #ccc; padding: 5px; margin-bottom: 10px; border-radius: 5px; width: 300px;" readonly></p>';
