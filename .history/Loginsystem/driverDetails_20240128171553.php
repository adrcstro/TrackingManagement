...
if (isset($_GET['search'])) {
    ...
    $sql = "SELECT Username, Age, Password, PhoneNumber, HomeAddress, DriversLicense, VehicleRegistration, PermittoOperate,Profile FROM driverstbl WHERE Username LIKE '%$searchText%' OR Age LIKE '%$searchText%' OR Password LIKE '%$searchText%'";
...
echo '<p><input type="text" id="homeAddress" value="Home Address: ' . $row["HomeAddress"] . '" readonly style="border: 1px solid #ccc; padding: 5px; margin-bottom: 10px; border-radius: 5px; width: 300px;"></p>';
...
echo '</div>';
echo '<div>';
echo '<img src="uploads/' . $row['PermittoOperate'] . '" style="border: 1px solid #ccc; border-radius: 5px; margin-top: 7rem; margin-right: 3rem; width: 350px; height: 300px;">';
echo '</div>';
echo '</div>';
      }
    } else {
