...
echo ' <input type="text" value="Driver Name: ' . $row["Username"] . '" class="form-control" id="driverNameInput" aria-describedby="button-addon2">';
echo ' <button class="btn btn-outline-secondary" type="button" id="copyButton" onclick="copyDriverName()">Select</button>';
echo '</div>';
...
<script>
function copyDriverName() {
  // Get the input element
  var driverNameInput = document.getElementById("driverNameInput");

  // Select the text in the input field
  driverNameInput.select();
  driverNameInput.setSelectionRange(0, 99999); // For mobile devices

  // Copy the text to the clipboard
  document.execCommand("copy");

  // Display a sweet alert indicating successful copy
  Swal.fire({
    title: "Copied!",
    text: "Driver name copied successfully.",
    icon: "success",
    timer: 2000, // Set the alert to disappear after 2 seconds
