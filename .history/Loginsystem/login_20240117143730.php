...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($usertype == 'admin') {
        ...
        $column_name = "Username, DriverID"; // Adjust column name for driver
    } elseif ($usertype == 'passenger') {
        $table_name = "passengertbl";
        $column_name = "Username, PassengerID"; // Adjust column name for passenger
    }
    ...
    if ($result->num_rows > 0) {
        ...
        $redirectUrl = '';
        $loggedInUserID = ''; // Variable to store the user ID

        if ($usertype == 'passenger') {
            $redirectUrl = "passengerdash.php?username=" . urlencode($row['Username']);
            $loggedInUserID = $row['PassengerID'];
        } elseif ($usertype == 'admin') {
            $redirectUrl = "admin.php";
        } elseif ($usertype == 'driver') {
            $redirectUrl = "driverdash.php?username=" . urlencode($row['Username']);
            $loggedInUserID = $row['DriverID'];
        }

        // Use $loggedInUserID as needed (e.g., storing in session for future use)
