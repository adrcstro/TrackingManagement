...
while($row = $result->fetch_assoc()) {
    // Check if ComplainantName matches any entry in the "Username" column of "driverstbl"
    $complainantName = $row["ComplainantName"];
    $checkQuery = "SELECT * FROM driverstbl WHERE Username = '$complainantName'";
    $checkResult = $conn->query($checkQuery);

    if ($checkResult->num_rows > 0) {
        // If there is a match, display the button
        echo '<tr>
                ...
                <td>
                    <button type="button" class="btn btn-secondary btn-sm w-100 mb-2" data-toggle="modal" data-target="#drivercomplainreport">
                        <i class="bi bi-file-text"></i> Driver to passenger Report
                    </button>
                </td>
            </tr>';
    } else {
        // If there is no match, display the column without the button
        echo '<tr>
                <td>' . $row["ComplaintID"] . '</td>
                <td>' . $row["TypeofComplaint"] . '</td>
                <td>' . $row["DateofReport"] . '</td>
                ...
                <td>' . $row["NameofComplainee"] . '</td>
                <td>' . $row["ComplainStatus"] . '</td>
                <td></td> <!-- Empty cell without the button -->
            </tr>';
    }
} 
$conn->close();
...
<button type="button" class="btn btn-secondary  btn-sm w-100 mb-2" data-toggle="modal" data-target="#ComplaintView">
    <i class="bi bi-file-text"></i> Passenger to Driver Report
</button>
