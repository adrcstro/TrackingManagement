...
<script>
        // Function to check the modal state in localStorage
        function checkModalState() {
            const modalState = localStorage.getItem('ratingModalState');
            if (modalState === 'open') {
                $('#RatingModal').modal('show');
            }
        }

        // Function to save the modal state to localStorage
        function saveModalState(state) {
            localStorage.setItem('ratingModalState', state);
        }

        $(document).ready(function () {
            // Check the modal state when the page loads
            checkModalState();

            // Show the modal when the button is clicked
            $('#openModalBtn').click(function () {
                $('#RatingModal').modal('show');
                // Save the modal state as open
                saveModalState('open');
            });

            // Save the modal state when the close button is clicked
            $('#closeModalBtn').click(function () {
                $('#RatingModal').modal('hide');
                // Save the modal state as closed
                saveModalState('closed');
            });

            // Save the modal state when it's about to be hidden (except when closed by the close button)
            $('#RatingModal').on('hide.bs.modal', function (e) {
                if (e.target.id !== 'closeModalBtn') {
                    saveModalState('open');
                    return false; // Prevent modal from being hidden
                }
            });
        });
    </script>
...
<button type="button" class="btn btn-secondary btn-sm m-1" id="openModalBtn">
        <i class="bi bi-clipboard-data"></i> Ratings
    </button>
