...
if (isset($_GET['Username'])) {
    ...
    if ($passengerResult->num_rows > 0) {
        ...
        if ($complaintResult->num_rows > 0) {
            // Complaints found, populate dropdown
            while ($row = $complaintResult->fetch_assoc()) {
                echo '<option value="' . $row["ComplaintID"] . '">' . $row["ComplaintID"];
                echo ($row["ComplaintDescription"]) ? ' - ' . $row["ComplaintDescription"] : '';  // Add the description if available
                echo '</option>';
            }
        } else {
            echo "No complaints found for the passenger.";
