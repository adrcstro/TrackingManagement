...
<table class="table table-hover table-nowrap" >
<?php
// Start the session (make sure to call this at the beginning of the script)
session_start();

// Check if the user is logged in as a driver
if (isset($_SESSION['usertype']) && $_SESSION['usertype'] === 'driver') {
    // Replace with your actual database credentials
    // Create connection
    $conn = new mysqli($servername, $username, $password, $dbname);
    ...
    // Fetch data for the logged-in driver
    $username = $_SESSION['username'];
    $sql = "SELECT * FROM driverstbl WHERE Username = '$username'";
    $result = $conn->query($sql);
    ...
    if ($result->num_rows > 0) {
        ...
        // Output data of the logged-in driver
        $row = $result->fetch_assoc();
        echo '<tr>
                <!-- Table data -->
             </tr>';
    ...
    // Redirect to login page if not logged in as a driver
