...
echo '<button class="btn btn-outline-secondary transparent-btn" type="button" onclick="viewFile(\'uploads/' . $row["DriversLicense"] . '\')">View <i class="fas fa-eye"></i></button>';
echo '</div>';
echo '</div>';
...
echo '<div class="input-group-append">';
echo '<button class="btn btn-outline-secondary transparent-btn" type="button" onclick="viewFile(\'uploads/' . $row["VehicleRegistration"] . '\')">View <i class="fas fa-eye"></i></button>';
echo '</div>';
echo '</div>';
...
function viewFile(filePath) {
    // Use AJAX to fetch and display the file content
    var xhr = new XMLHttpRequest();
    xhr.onreadystatechange = function() {
        if (xhr.readyState == 4 && xhr.status == 200) {
            alert("File Content:uploads/" + xhr.responseText);
            // You can modify this part to display the file content in a modal or another element
        }
    };
