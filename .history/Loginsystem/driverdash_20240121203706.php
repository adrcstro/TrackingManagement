...
// Assuming you have $driverNameParam available (replace it with your actual parameter)
if (isset($driverNameParam)) {
    // Create connection
    $conn = new mysqli($servername, $username, $password, $dbname);
    ...
    $sql = "SELECT DriverName, Rate FROM rating WHERE DriverName = ?";
    $stmt = $conn->prepare($sql);
    ...
    $stmt->bind_param("s", $driverNameParam);
    $stmt->execute();
    $result = $stmt->get_result();
    ...
    if ($result->num_rows > 0) {
        ...
        echo "No data found for the specified driver in the rating table.";
    }
