...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            // Change the font size for the COMPLAINED REPORT
            $pdf->SetFontSize(16);
        
            // Calculate the X-axis position to center the text
            $xCentered = ($pdf->GetPageWidth() - $pdf->GetStringWidth("COMPLAINED REPORT")) / 2;
            
            // Set the X-axis position
            $pdf->SetX($xCentered);
        
            // Output the centered text
            $pdf->MultiCell(0, 12, "COMPLAINED REPORT\n\n");
        
            // Reset the font size for subsequent content
            $pdf->SetFontSize(12);
