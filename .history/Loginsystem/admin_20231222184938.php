    ...
    <?php
        ...
        if ($result) {
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                // Move time and date above the card title
                echo '<div class="cardtimetobody">';
                echo '<p  class="card-text mt-1"><small class="text-muted">' . $time . ' | ' . $date . '</small></p>';
                
                $trimmed_body = strlen($body) > 90 ? substr($body, 0, 90) . '...' : $body;
                echo '<p class="card-text mt-1">' . $trimmed_body . ' <a href="#" class="read-more" data-toggle="modal" data-target="#readMoreModal' . $row['NewsID'] . '">Read More</a></p>';
                echo '</div>';
                echo '</div>';
                echo '</div>';
                echo '</div>';
