...
if (isset($_GET['search'])) {
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            echo '<p style="position: relative;"><input type="text" value="Driver Name: ' . $row["Username"] . '" style="border: 1px solid #ccc; padding: 5px; margin-bottom: 10px; margin-top: 20px; border-radius: 5px; width: 300px;" readonly>';
            echo '<button style="background-color: gray; padding: 5px; border: none; border-radius: 5px; color: white; position: absolute; right: 5px; top: 5px;">View gray bg</button></p>';

            echo '<p style="position: relative;"><input type="text" value="Vehicle Number: ' . $row["Age"] . '" style="border: 1px solid #ccc; padding: 5px; margin-bottom: 10px; border-radius: 5px; width: 300px;" readonly>';
            echo '<button style="background-color: gray; padding: 5px; border: none; border-radius: 5px; color: white; position: absolute; right: 5px; top: 5px;">View gray bg</button></p>';

            echo '<p style="position: relative;"><input type="text" value="Plate Number: ' . $row["Password"] . '" style="border: 1px solid #ccc; padding: 5px; margin-bottom: 10px; border-radius: 5px; width: 300px;" readonly>';
            echo '<button style="background-color: gray; padding: 5px; border: none; border-radius: 5px; color: white; position: absolute; right: 5px; top: 5px;">View gray bg</button></p>';
