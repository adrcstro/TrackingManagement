...
if (isset($_GET['Username'])) {
    ...
    // Assuming you have the database connection parameters defined
    $conn = new mysqli($servername, $username, $password, $dbname);
    ...
    // Fetching the Rate data from the rating table
    $sql = "SELECT Rate FROM rating WHERE DriverUsername = ?";
    $stmt = $conn->prepare($sql);
    ...
    if ($result->num_rows > 0) {
        ...
        // Fetching the rates and storing them in an array
        while ($row = $result->fetch_assoc()) {
            $rates[] = $row['Rate'];
        }
        ...
        foreach ($rates as $rate) {
            echo '<div class="bar" style="height: ' . $rate . 'px;"></div>';
        }
        echo '</div>';
    } else {
        echo "No ratings available for the driver.";
    }
