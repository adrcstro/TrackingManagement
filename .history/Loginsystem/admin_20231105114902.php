...
?>
    ...
    <button type="button" class="btn btn-warning btn-sm m-1" data-toggle="modal" data-target="#update" id="updateButton" onclick="performUpdate()">
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $selected_admin = $_POST['input1']; // Assuming this is the value of the selected admin

    $updated_username = $_POST['input2'];
    $updated_date = $_POST['datePicker'];
    $updated_password = $_POST['input3'];

    // Check if the selected_admin is not empty before proceeding with the update
    if (!empty($selected_admin)) {
        $sql = "UPDATE admintbl SET Username='$updated_username', Password='$updated_password', DateCreated='$updated_date' WHERE Admin='$selected_admin'";

        if ($conn->query($sql) === TRUE) {
            echo "Record updated successfully";

            // Display the updated values in the database
            $sql = "SELECT * FROM admintbl WHERE Admin='$selected_admin'";
            $result = $conn->query($sql);

            if ($result->num_rows > 0) {
                while($row = $result->fetch_assoc()) {
                    echo "Admin: " . $row["Admin"] . " - Username: " . $row["Username"] . " - Date Created: " . $row["DateCreated"];
                }
            } else {
                echo "0 results";
            }
...
function performUpdate() {
    var selected_admin = ""; // Define the selected admin here

    var updated_username = ""; // Define the updated username here
    var updated_date = ""; // Define the updated date here
    var updated_password = ""; // Define the updated password here

    var xhr = new XMLHttpRequest();
    var url = 'your_php_script.php';
    var params = 'input1=' + selected_admin + '&input2=' + updated_username + '&datePicker=' + updated_date + '&input3=' + updated_password;
