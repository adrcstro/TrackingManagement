...
<script>
    ...
    function fetchAndRenderLineGraph() {
        ...
        var username = "root";
        var password = "";
        var database = "plate-to-place-v-tracking";
        ...
        if (yourDbConnection) {
            ...
            var query = "SELECT Rate FROM rating";
            yourDbConnection.query(query, function (err, result) {
                if (err) throw err;

                // Extract data for the line graph
                var labels = result.map(function (row) {
                    return row.Date; // Assuming there's a column named "Date" in the rating table
                });
