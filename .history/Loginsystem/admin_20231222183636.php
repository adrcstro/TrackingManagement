    ...
    <?php
        ...
        if ($result) {
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                echo '<p class="card-text mt-2"><small class="text-muted">' . $time . ' | ' . $date . '</small></p>'; // Added mt-2 class for top margin
                $trimmed_body = strlen($body) > 90 ? substr($body, 0, 90) . '...' : $body;
                echo '<p class="card-text mt-2">' . $trimmed_body . ' <a href="#" class="read-more" data-toggle="modal" data-target="#readMoreModal' . $row['NewsID'] . '">Read More</a></p>'; // Added mt-2 class for top margin
                echo '</div>';
                echo '</div>';
                echo '</div>';
